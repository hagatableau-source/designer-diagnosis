...
export const observationQuestions: ObservationQuestion[] = [
  ...
  // Phase3：3つ選択（認知特性の抽出）
  {
    ...
    choices: [
      ...
      { text: "安全のしおりに書かれた文字の羅列", axis: "dummy" }, // 感性ゼロ：単なる業務的な規定（情報）
      { text: "自分のスマホの機内モードの設定画面", axis: "dummy" }, // 感性ゼロ：単なる通信上のルール（操作）
      { text: "座席の前の網ポケットに入ったゴミ袋", axis: "dummy" }, // 感性ゼロ：ただの物理的な配置（無関心）
      { text: "手元の時計が刻んでいる現在の時刻", axis: "dummy" }  // 感性ゼロ：空間と無関係な数字の確認（時間）
    ]
  },
  {
    id: 202,
  ...
  title: "朝の服選び。あなたが最後の「決め手」にするのは？",
    choices: [
      ...
      { text: "タンスのクリップの一番手前にあるという事実", axis: "dummy" }, // 感性ゼロ：単なる物理的な配置（無関心）
      { text: "今日の天気予報に表示された最高気温の数字", axis: "dummy" }, // 感性ゼロ：単なるデータへの適応（情報）
      { text: "昨日からハンガーに掛けっぱなしだった状態", axis: "dummy" }, // 感性ゼロ：単なる前日の惰性の維持（状況）
      { text: "出かけるまでに残されたあと数分という時間", axis: "dummy" }  // 感性ゼロ：単なる時間の制約（外因）
    ]
  },
  {
    id: 203,
   ...
   title: "古い喫茶店のカウンター席。ついあなたの目が奪われるのは？",
    choices: [
      ...
      { text: "テーブルの端に貼られた伝票", axis: "dummy" },  // 感性ゼロ：単なる事務的な支払いの紙（記号）
      { text: "メニュー表に記載された価格の数字", axis: "dummy" }, // 感性ゼロ：単なるコストの計算（経済）
      { text: "スマホの画面に表示された現在時刻", axis: "dummy" }, // 感性ゼロ：ただの時間の経過確認（情報）
      { text: "店内の壁に設置された避難口の標識", axis: "dummy" }  // 感性ゼロ：単なる記号的な設備の視認（状況）
    ]
  },
  {
    id: 204,
 ...
 title: "美術館の展示。作品そのものより、あなたが「つい気にしてしまう」のは？",
    choices: [
      ...
      { text: "館内パンフレットの文字の羅列", axis: "dummy" },  // 感性ゼロ：単なる印刷された記号の確認（情報）
      { text: "ミュージアムショップの営業時間", axis: "dummy" }, // 感性ゼロ：ただの買い物タスクへの意識（予定）
      { text: "歩きすぎて少し痛む足の裏の感覚", axis: "dummy" }, // 感性ゼロ：単なる肉体的な疲労（現状・外因）
      { text: "出口を出た後の昼食のメニュー", axis: "dummy" }  // 感性ゼロ：今ここの空間からの完全な意識逃避（欲求）
    ]
  },
  {
    id: 205,
    phase: "add",
title: "満員のエレベーター。他人と密着する空間で、あなたが見てしまうのは？",
    choices: [
      ...
      { text: "スマホに届いている無関係な通知の文字", axis: "dummy" }, // 感性ゼロ：目の前の空間からの完全な逃避（情報）
      { text: "昨日見たテレビ番組のどうでもいい一場面", axis: "dummy" }, // 感性ゼロ：脳内での過去の再生（記憶）
      { text: "ただ床の一点を見つめて硬直している視界", axis: "dummy" }, // 感性ゼロ：思考と視覚の完全なシャットダウン（停止）
      { text: "ポケットの中の小銭を指先で数える作業", axis: "dummy" }  // 感性ゼロ：手元の単純動作への意識集中（タスク）
    ]
  },
  {
    id: 214, // 例
    ...
    choices: [
      ...
      { text: "今いる場所から一番距離が近い自動販売機", axis: "dummy" },  // 感性ゼロ：単なる物理的な最短距離（空間）
      { text: "オフィスに備え付けられている無料の給茶機", axis: "dummy" }, // 感性ゼロ：単なるコストゼロの選択（経済）
      { text: "カバンの中に残っている、いつ買ったか分からない水", axis: "dummy" }, // 感性ゼロ：単なる手元の水分補給（生存）
      { text: "次のミーティングまでのわずか3分という時間", axis: "dummy" }  // 感性ゼロ：単なるスケジュールの隙間（時間）
    ]
  },
  {
    id: 207,
    ...
    choices: [
      ...
      { text: "上司から指定された締め切り時間", axis: "dummy" },  // 感性ゼロ：単なるスケジュールの順守（時間）
      { text: "社内規定で決まっているフォーマット", axis: "dummy" }, // 感性ゼロ：単なる既存ルールの適応（外因）
      { text: "パソコンのバッテリーの残り残量", axis: "dummy" },  // 感性ゼロ：単なる機材の状況（環境）
      { text: "この作業が終わった後の次の予定", axis: "dummy" }  // 感性ゼロ：単なるタスクの消化（意識の逃避）
    ] 
  },
  {
    id: 208,
  ...
  title: "歴史ある古い建物。その前に立った時、あなたが「つい見つめてしまう」のは？",
    choices: [
      ...
      { text: "案内板に書かれた建物の名称", axis: "dummy" },  // 感性ゼロ：単なる文字情報の確認（記号）
      { text: "入場料がいくらであるかという事実", axis: "dummy" }, // 感性ゼロ：単なるコストの計算（経済）
      { text: "ここが観光ルートの何番目かという順序", axis: "dummy" }, // 感性ゼロ：単なるスケジュールの消化（タスク）
      { text: "今持っている荷物の重さや自分の疲労", axis: "dummy" }  // 感性ゼロ：単なる肉体的な現状（内省・外因）
    ]
  },
  {
    id: 209,
...
title: "差し出された名刺を受け取った瞬間、あなたの目が真っ先に向かうのは？",
    choices: [
      ...
      // --- 【ダミー：名刺への感性が完全にゼロである状態を吸い取る4択】 ---
      { text: "単なる「業務上の連絡先」という事実", axis: "dummy" },  // 感性ゼロ：ただのデータ
      { text: "名刺交換という一連のルーティン作業", axis: "dummy" },  // 感性ゼロ：ただの習慣（自動化）
      { text: "後でデータベースに登録するというタスク", axis: "dummy" }, // 感性ゼロ：ただの後処理（労働）
      { text: "受け取ってファイリングするまでの手順", axis: "dummy" }  // 感性ゼロ：ただの整理（作業）
    ]
  },
  {
    id: 210,
    ...
    { text: "特に何も考えず、すぐに書く", axis: "dummy" },  // 認知の停止（無関心）
    { text: "今日が何月何日であるかという事実", axis: "dummy" }, // 事務的な記録（情報）
    { text: "今、手元にこのノートしかない状況", axis: "dummy" }, // 単なる状況の強制（外因）
    { text: "誰かに早く書けと急かされている感覚", axis: "dummy" } // 他者からの強制（外因）
    ]
  },
];